...
function cross(vec0: any, vec1: any) {
    return vec0.x * vec1.y - vec0.y * vec1.x;
}
function getPointOrientation(edge: any, p: any) {
...
function binSorter(a: any, b: any) {
    if (a.bin == b.bin) {
        return 0;
    }
    else {
...
function isQuadConvex(p0: any, p1: any, p2: any, p3: any) {
    const diag0 = [p0, p2];
    const diag1 = [p1, p3];
    return isEdgeIntersecting(diag0, diag1);
}
function isSameEdge(edge0: any, edge1: any) {
    return ((edge0[0] == edge1[0] && edge0[1] == edge1[1]) ||
        (edge0[1] == edge1[0] && edge0[0] == edge1[1]));
}
function isEdgeIntersecting(edgeA: any, edgeB: any) {
...
function setupDelaunay(meshData: any) {
...
function delaunay(meshData: any) {
...
function findEnclosingTriangle(target_vertex: any, meshData: any, ind_tri_cur: any) {
...
function restoreDelaunay(ind_vert: any, meshData: any, stack: any) {
    const vertices = meshData.scaled_vert;
    const triangles = meshData.tri;
    const adjacency = meshData.adj;
...
function swapDiagonal(meshData: any, ind_triA: any, ind_triB: any) {
    const triangles = meshData.tri;
    const adjacency = meshData.adj;
...
function removeBoundaryTriangles(meshData: any) {
    const verts = meshData.scaled_vert;
    const triangles = meshData.tri;
    const adjacency = meshData.adj;
...
function isDelaunay2(v_tri: any, p: any) {
...
function constrainEdges(meshData: any) {
...
function buildVertexConnectivity(meshData: any) {
    const triangles = meshData.tri;
    meshData.vert_to_tri = [];
...
function getEdgeIntersections(meshData: any, iedge: any) {
    const triangles = meshData.tri;
    const verts = meshData.scaled_vert;
    const adjacency = meshData.adj;
    const con_edges = meshData.con_edge;
    ...
    if (!edge_in_triangulation) {
        ...
        while (true) {
            ...
            // @ts-expect-error ts-migrate(7022) FIXME: 'prev_edge_ind' implicitly has type 'any' because ... Remove this comment to see the full error message
...
function fixEdgeIntersections(meshData: any, intersectionList: any, con_edge_ind: any, newEdgeList: any) {
    const triangles = meshData.tri;
    const verts = meshData.scaled_vert;
    const adjacency = meshData.adj;
    const con_edges = meshData.con_edge;
...
function loadEdges(meshData: any, edges: any) {
    const nVertex = meshData.vert.length;
    meshData.con_edge = [];
...
function isEdgeValid(newEdge: any, edgeList: any, vertices: any) {
...
export default function (points: any, edges: any, z: any) {
    if (!edges)
        edges = [];
    ...
    // Normalize points
    const normPoints = points.features.map((point: any) => {
        const xy = point.geometry.coordinates;
        const normXy = [
    ...
    const keys = ["a", "b", "c"];
    return featureCollection((meshData as any).tri.map((indices: any) => {
        const properties = {};
        const coords = indices.map((index: any, i: any) => {
