...
(function () {
  ...
  utils.some = function (arr, test) {
    return arr.reduce(
      (val, item) => val || test(item), // TODO: short-circuit?
      false
    );
  };
...
*/
  ...
  function ArcCollection(...args) {
    let _xx,
      _yy, // coordinates data
      _ii,
      _nn, // indexes, sizes
      _zz,
      _zlimit = 0, // simplification
      _bb,
      _allBounds, // bounding boxes
      _arcIter,
  ...
  geom.debugSegmentIntersection = function (p, ax, ay, bx, by, cx, cy, dx, dy) {
    ...
    debug(
      "  angle:",
      geom.signedAngle(ax, ay, bx, by, dx - cx + bx, dy - cy + by)
    );
  };
...
*/
  ...
  internal.traversePaths = function traversePaths(
    shapes,
    cbArc,
    cbPart,
    cbShape
