...
class Tin {
    ...
    setCompiled(compiled: any) {
        if (!compiled.tins && compiled.points && compiled.tins_points) {
            ...
            this.vertices_params.forw[1] = [0, 1, 2, 3].map(idx => {
                ...
                const tri = indexesToTri(["cent", `bbox${idx}`, `bbox${idxNxt}`], compiled.points, compiled.edgeNodes || [], compiled.centroid_point, compiled.vertices_points, false);
                return featureCollection([tri]);
            });
            this.vertices_params.bakw[1] = [0, 1, 2, 3].map(idx => {
                const idxNxt = (idx + 1) % 4;
                const tri = indexesToTri(["cent", `bbox${idx}`, `bbox${idxNxt}`], compiled.points, compiled.edgeNodes || [], compiled.centroid_point, compiled.vertices_points, true);
                return featureCollection([tri]);
            });
            // centroidを復元
            this.centroid = {
            ...
            this.tins = {
                forw: featureCollection(compiled.tins_points[0].map((idxes: any) => indexesToTri(idxes, compiled.points, compiled.edgeNodes || [], compiled.centroid_point, compiled.vertices_points, false))),
                bakw: featureCollection(compiled.tins_points[bakwI].map((idxes: any) => indexesToTri(idxes, compiled.points, compiled.edgeNodes || [], compiled.centroid_point, compiled.vertices_points, true)))
            };
            this.addIndexedTin();
            // kinksを復元
            if (compiled.kinks_points) {
                this.kinks = {
                    bakw: featureCollection(compiled.kinks_points.map((coord: any) => point(coord)))
                };
            }
            // yaxisModeを復元
    ...
    getCompiled() {
        ...
        this.tins.forw.features.map((tin: any) => {
            (compiled as any).tins_points[0].push(["a", "b", "c"].map(idx => tin.properties[idx].index));
        });
        ...
        if (this.strict_status == Tin.STATUS_LOOSE) {
            ...
            this.tins.bakw.features.map((tin: any) => {
                (compiled as any).tins_points[1].push(["a", "b", "c"].map(idx => tin.properties[idx].index));
            });
        }
        else if (this.strict_status == Tin.STATUS_ERROR) {
            (compiled as any).kinks_points = this.kinks.bakw.features.map((kink: any) => kink.geometry.coordinates);
        }
        // yaxisMode対応
        if (this.yaxisMode == Tin.YAXIS_FOLLOW) {
    ...
    addIndexedTin() {
        ...
        const forwGridCache = forwEachBound.reduce((prev: any, bound: any, index: any) => {
        ...
        const bakwGridCache = bakwEachBound.reduce((prev: any, bound: any, index: any) => {
    ...
    calcurateStrictTinAsync() {
        ...
        return Promise.all(self.tins.forw.features.map((tri: any) => Promise.resolve(counterTri(tri))))
            .then(tris => {
            ...
            .then(() => {
            const searchIndex = {};
            return Promise.all(self.tins.forw.features.map((forTri: any, index: any) => {
            ...
            if ((overlapped as any).bakw)
                Object.keys((overlapped as any).bakw).map(key => {
                    if ((overlapped as any).bakw[key] == "Not include case")
                        return;
                    ...
                    const nonSharedVtx = trises.map((tris: any) => ["a", "b", "c"]
                        .map((alpha, index) => {
            ...
            return Promise.all(["forw", "bakw"].map(direc => new Promise(resolve => {
                const coords = self.tins[direc].features.map((poly: any) => poly.geometry.coordinates[0]);
    ...
    generatePointsSet() {
        ...
        for (let i = 0; i < self.edges.length; i++) {
            ...
            const lengths = [illstNodes, mercNodes].map(nodes => {
                ...
                const sumLengths = eachLengths.reduce((prev: any, node: any, index: any) => {
                    if (index === 0)
                        return [0];
            ...
            lengths
                ...
                return thisLengths
                    .filter((val: any, index: any) => index === 0 ||
                    ...
                    const anotherSets = anotherLengths.reduce((prev: any, item: any, index: any, arr: any) => {
                        if (prev)
                            return prev;
    ...
    updateTinAsync() {
        ...
        const insideCheck = this.bounds
            ...
            : (xy: any) => xy[0] >= self.xy[0] &&
                xy[0] <= self.xy[0] + self.wh[0] &&
                xy[1] >= self.xy[1] &&
                xy[1] <= self.xy[1] + self.wh[1];
        const inside = this.points.reduce((prev: any, curr: any) => prev && insideCheck(curr[0]), true);
        if (!inside) {
    ...
    transform(apoint: any, backward: any, ignoreBounds: any) {
    ...
    calculatePointsWeightAsync() {
        ...
        if (self.strict_status == Tin.STATUS_LOOSE)
        ...
        return Promise.all(calcTargets.map(target => {
            ...
            return Promise.all(tin.features.map((tri: any) => {
                const vtxes = ["a", "b", "c"];
                return new Promise(resolve => {
                    for (let i = 0; i < 3; i++) {
                        const j = (i + 1) % 3;
...
// eslint-disable-next-line no-unused-vars
function useVertices(o: any, verticesParams: any, centroid: any, weightBuffer: any) {
    return point(useVerticesArr(o, verticesParams, centroid, weightBuffer));
}
function useVerticesArr(o: any, verticesParams: any, centroid: any, weightBuffer: any) {
...
// eslint-disable-next-line no-unused-vars
function transform(point: any, tins: any, indexedTins: any, verticesParams: any, centroid: any, weightBuffer: any, stateTriangle: any, stateSetFunc: any) {
    return point(transformArr(point, tins, indexedTins, verticesParams, centroid, weightBuffer, stateTriangle, stateSetFunc));
}
function transformArr(point: any, tins: any, indexedTins: any, verticesParams: any, centroid: any, weightBuffer: any, stateTriangle: any, stateSetFunc: any) {
    let tin;
    if (stateTriangle) {
...
function indexesToTri(indexes: any, points: any, edgeNodes: any, cent: any, bboxes: any, bakw: any) {
...
function removeSearchIndex(searchIndex: any, tris: any, tins: any) {
    ...
    for (let i = 0; i < keys.length; i++) {
        const key = keys[i];
        const newArray = searchIndex[key].filter((eachTris: any) => eachTris.forw != tris.forw);
    ...
    if (tins) {
        let newArray = tins.forw.features.filter((eachTri: any) => eachTri != tris.forw);
        tins.forw.features = newArray;
        newArray = tins.bakw.features.filter((eachTri: any) => eachTri != tris.bakw);
