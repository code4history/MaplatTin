...
import { intersectSegments } from '@/edgebound/base';
...
const mogrifiers: { [key: string]: (pts: P2x4, ret: boolean) => [P2x4, boolean] } = {
  id: (pts, ret) => [pts, ret],
  // reverse p1 <-> p2
...
function testIntersect(pts: P2[], should: boolean) {
  const [[p1x, p1y], [p2x, p2y], [p3x, p3y], [p4x, p4y]] = pts;
  const ret = intersectSegments(p1x, p1y, p2x, p2y, p3x, p3y, p4x, p4y);
  expect(ret).toBe(should);
}

describe('Intersection Tests', () => {
  const intFns = [...Object.values(mogrifiers)];

  ints.forEach(([should, name, ...pts]) => {
    describe(name, () => {
      intFns.forEach((fn, idx) => {
        it(`should handle transformation ${Object.keys(mogrifiers)[idx]}`, () => {
          testIntersect(...fn(pts, should));
        });
      });
    });
  });
