...
function () {
  ...
  _createClass(Tin, [{
    ...
    key: "addIndexedTin",
    value: function addIndexedTin() {
      var tins = this.tins;
      var forw = tins.forw;
      var bakw = tins.bakw;
      var gridNum = Math.ceil(Math.sqrt(forw.features.length));
      ...
      var forwBound = null;
      var bakwBound = null;
      var forwEachBound = forw.features.map(function (tri) {
        var eachBound = null;
        (0, _invariant.getCoords)(tri)[0].map(function (point) {
          if (forwBound == null) forwBound = [Array.from(point), Array.from(point)];else {
          ...
          if (eachBound == null) eachBound = [Array.from(point), Array.from(point)];else {
      ...
      var forwXUnit = (forwBound[1][0] - forwBound[0][0]) / gridNum;
      var forwYUnit = (forwBound[1][1] - forwBound[0][1]) / gridNum;
      var forwGridCache = forwEachBound.reduce(function (prev, bound, index) {
        var normXMin = unitCalc(bound[0][0], forwBound[0][0], forwXUnit, gridNum);
        var normXMax = unitCalc(bound[1][0], forwBound[0][0], forwXUnit, gridNum);
        var normYMin = unitCalc(bound[0][1], forwBound[0][1], forwYUnit, gridNum);
        var normYMax = unitCalc(bound[1][1], forwBound[0][1], forwYUnit, gridNum);

        for (var cx = normXMin; cx <= normXMax; cx++) {
          if (!prev[cx]) prev[cx] = [];

          for (var cy = normYMin; cy <= normYMax; cy++) {
      ...
      var bakwEachBound = bakw.features.map(function (tri) {
        var eachBound = null;
        (0, _invariant.getCoords)(tri)[0].map(function (point) {
          if (bakwBound == null) bakwBound = [Array.from(point), Array.from(point)];else {
      ...
      var bakwXUnit = (bakwBound[1][0] - bakwBound[0][0]) / gridNum;
      var bakwYUnit = (bakwBound[1][1] - bakwBound[0][1]) / gridNum;
      var bakwGridCache = bakwEachBound.reduce(function (prev, bound, index) {
        var normXMin = unitCalc(bound[0][0], bakwBound[0][0], bakwXUnit, gridNum);
        var normXMax = unitCalc(bound[1][0], bakwBound[0][0], bakwXUnit, gridNum);
        var normYMin = unitCalc(bound[0][1], bakwBound[0][1], bakwYUnit, gridNum);
        var normYMax = unitCalc(bound[1][1], bakwBound[0][1], bakwYUnit, gridNum);
    ...
    value: function transform(apoint, backward, ignoreBounds) {
      ...
      var indexedTins = backward ? this.indexedTins.bakw : this.indexedTins.forw;
      ...
      if (this.stateFull) {
        ...
        stateSetFunc = function stateSetFunc(tri) {
          _this2.stateTriangle = tri;
        };
      }

      var ret = transformArr(tpoint, tins, indexedTins, verticesParams, centroid, weightBuffer, stateTriangle, stateSetFunc);
...
function unitCalc(coord, origin, unit, gridNum) {
  var normCoord = Math.floor((coord - origin) / unit);
  if (normCoord >= gridNum) normCoord = gridNum - 1;
  return normCoord;
}

function transform(point, tins, indexedTins, verticesParams, centroid, weightBuffer, stateTriangle, stateSetFunc) {
  // eslint-disable-line no-unused-vars
...
function transformArr(point, tins, indexedTins, verticesParams, centroid, weightBuffer, stateTriangle, stateSetFunc) {
  ...
  if (!tin) {
    if (indexedTins) {
      ...
      var gridNum = indexedTins.gridNum;
      var xOrigin = indexedTins.xOrigin;
      var yOrigin = indexedTins.yOrigin;
      var xUnit = indexedTins.xUnit;
      var yUnit = indexedTins.yUnit;
      var gridCache = indexedTins.gridCache;
      var normX = unitCalc(coords[0], xOrigin, xUnit, gridNum);
      var normY = unitCalc(coords[1], yOrigin, yUnit, gridNum);
      var tinsKey = gridCache[normX] ? gridCache[normX][normY] ? gridCache[normX][normY] : [] : [];
      tins = {
        features: tinsKey.map(function (key) {
          return tins.features[key];
        })
      };
    }
