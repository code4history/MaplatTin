...
class Tin {
    ...
    setCompiled(compiled) {
        if (!compiled.tins && compiled.points && compiled.tins_points) {
            ...
            this.tins = {
                forw: featureCollection(compiled.tins_points[0].map((idxes) => indexesToTri(idxes, compiled.points, compiled.edgeNodes || [], compiled.centroid_point, compiled.vertices_points, false))),
                bakw: featureCollection(compiled.tins_points[bakwI].map((idxes) => indexesToTri(idxes, compiled.points, compiled.edgeNodes || [], compiled.centroid_point, compiled.vertices_points, true)))
            };
            this.addIndexedTin();
...
function transform(point, tins, indexedTins = undefined, verticesParams = undefined, centroid = undefined, weightBuffer = undefined, stateTriangle = undefined, stateSetFunc = undefined) {
    return point(transformArr(point, tins, indexedTins, verticesParams, centroid, weightBuffer, stateTriangle, stateSetFunc));
}
function transformArr(point, tins, indexedTins = undefined, verticesParams = undefined, centroid = undefined, weightBuffer = undefined, stateTriangle = undefined, stateSetFunc = undefined) {
    let tin;
    if (stateTriangle) {
...
function insertSearchIndex(searchIndex, tris, tins = undefined) {
