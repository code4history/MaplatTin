...
class Tin {
    ...
    calculatePointsWeightAsync() {
        ...
        return Promise.all(calcTargets.map(target => {
            ...
            .then(() => {
            const pointsWeightBuffer = { forw: {}, bakw: {} };
...
function transform(apoint, tins, indexedTins, verticesParams = undefined, centroid = undefined, weightBuffer, stateTriangle, stateSetFunc) {
    return point(transformArr(apoint, tins, indexedTins, verticesParams, centroid, weightBuffer, stateTriangle, stateSetFunc));
}
function transformArr(point, tins, indexedTins, verticesParams = undefined, centroid = undefined, weightBuffer, stateTriangle, stateSetFunc) {
    let tin;
    if (stateTriangle) {
