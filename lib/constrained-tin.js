...
function () {
  ...
  _createClass(Point, [{
    ...
    key: "sqDistanceTo",
    value: function sqDistanceTo(p1) {
      return (this.x - p1.x) * (this.x - p1.x) + (this.y - p1.y) * (this.y - p1.y);
    }
  }, {
    key: "toStr",
    value: function toStr() {
      return "(".concat(this.x.toFixed(3), ", ").concat(this.y.toFixed(3), ")");
    }
  }, {
    key: "copyFrom",
...
function setupDelaunay(meshData) {
  ...
  var nBinsX = Math.round(Math.pow(nVertex, 0.25)); //Compute scaled vertex coordinates and assign each vertex to a bin
  ...
  for (var _i = nVertex; _i < nVertex + 3; _i++) {
    meshData.vert.push(new Point(fieldSize * scaledverts[_i].x + fieldOrigin.x, fieldSize * scaledverts[_i].y + fieldOrigin.y));
...
function delaunay(meshData) {
  ...
  for (var i = 0; i < N; i++) {
    ...
    if (cur_tri_adj[0] >= 0) //if triangle N_tri's neighbor exists
      {
        //Find the index for cur_tri in the adjacency of the neighbor
        var _neigh_adj_ind = adjacency[cur_tri_adj[0]].indexOf(ind_tri);

        adjacency[cur_tri_adj[0]][_neigh_adj_ind] = N_tri;
        stack.push([cur_tri_adj[0], _neigh_adj_ind]);
      }

    if (cur_tri_adj[1] >= 0) //if triangle (N_tri+1)'s neighbor exists
      {
        //Find the index for cur_tri in the adjacency of the neighbor
        var _neigh_adj_ind2 = adjacency[cur_tri_adj[1]].indexOf(ind_tri);

        adjacency[cur_tri_adj[1]][_neigh_adj_ind2] = N_tri + 1;
        stack.push([cur_tri_adj[1], _neigh_adj_ind2]);
      }
...
function findEnclosingTriangle(target_vertex, meshData, ind_tri_cur) {
  ...
  if (!found_tri) {
    throw "Could not locate the triangle that encloses (".concat(target_vertex.x, ", ").concat(target_vertex.y, ")!");
  }
  ...
  while (stack.length > 0) {
    ...
    if (!isDelaunay2(v_tri, v_new)) {
      ...
      if (ind_tri_neigh_outer >= 0) {
        var _neigh_node = adjacency[ind_tri_neigh_outer].indexOf(ind_tri_neigh);

        stack.push([ind_tri_neigh_outer, _neigh_node]);
      }
    } //is not Delaunay
...
function swapDiagonal(meshData, ind_triA, ind_triB) {
  ...
  if (ind_triB_neigh_outerp1 >= 0) {
    var _neigh_node2 = adjacency[ind_triB_neigh_outerp1].indexOf(ind_triB);

    adjacency[ind_triB_neigh_outerp1][_neigh_node2] = ind_triA;
...
function removeBoundaryTriangles(meshData) {
  ...
  for (var _i2 = 0; _i2 < adjacency.length; _i2++) {
    for (var _j = 0; _j < 3; _j++) {
      adjacency[_i2][_j] = indmap[adjacency[_i2][_j]];
    }
...
function constrainEdges(meshData) {
  ...
  for (var iedge = 0; iedge < con_edges.length; iedge++) {
    ...
    if (intersections.length > 0) throw "Could not add edge ".concat(iedge, " to triangulation after ").concat(maxIter, " iterations!");
  ...
  while (true) {
    ...
    for (var _iedge = 0; _iedge < newEdgeList.length; _iedge++) {
      var new_edge_nodes = newEdgeList[_iedge]; //Check if the new edge is a constrained edge
      ...
      if (!isDelaunay2(triA_verts, triB_vert)) {
        ...
        newEdgeList[_iedge] = [triangles[tri_ind_pair[0]][outer_nodeA_ind], triangles[tri_ind_pair[1]][outer_nodeB_ind]];
      }
    } //loop over new edges
...
function getEdgeIntersections(meshData, iedge) {
  ...
  if (!edge_in_triangulation) {
    ...
    while (true) {
      ...
      var _cur_tri = triangles[tri_ind]; //Loop over the other two edges in this triangle,
      //and check if they intersect the constrained edge

      for (var offset = 1; offset < 3; offset++) {
        var _v0_node = (prev_edge_ind + offset + 1) % 3;

        var v1_node = (prev_edge_ind + offset + 2) % 3;
        var cur_edge_coords = [verts[_cur_tri[_v0_node]], verts[_cur_tri[v1_node]]];
...
function loadEdges(meshData, edges) {
  ...
  for (var i = 0; i < edges.length; i++) {
    ...
    if (edge[0] < 0 || edge[0] >= nVertex || edge[1] < 0 || edge[1] >= nVertex) {
      throw "Vertex indices of edge ".concat(i, " need to be non-negative and less than the number of input vertices."); // meshData.con_edge = [];
      // break;
    }

    if (edge[0] === edge[1]) {
      throw "Edge ".concat(i, " is degenerate!"); // meshData.con_edge = [];
      // break;
    }

    if (!isEdgeValid(edge, meshData.con_edge, meshData.vert)) {
      throw "Edge ".concat(i, " already exists or intersects with an existing edge!"); // meshData.con_edge = [];
      // break;
    }
...
function _default(points, edges, z) {
  if (!edges) edges = [];
  ...
  var keys = ['a', 'b', 'c'];
  return (0, _helpers.featureCollection)(meshData.tri.map(function (indices) {
    var properties = {};
    ...
    coords[3] = coords[0];
    return (0, _helpers.polygon)([coords], properties);
  }));
}
