...
(function () {
  ...
  var utils = {
    ...
    addslashes: function addslashes(str) {
      return "".concat(str).replace(/[\\"']/g, '\\$&').replace(/\u0000/g, '\\0'); // eslint-disable-line no-control-regex
    },
    ...
    defaults: function defaults(dest) {
      for (var i = 0, n = arguments.length <= 1 ? 0 : arguments.length - 1; i < n; i++) {
    ...
    inherit: function inherit(targ, src) {
      var f = function f() {
        if (this.__super__ == f) {
          ...
          src.apply.apply(src, [this].concat(args)); // remove temp __super__, expose targ.prototype.__super__ again
    ...
    subclass: function subclass(parent) {
      ...
      var child = function child() {
        this.__super__.apply(this, [parent].concat(_toConsumableArray(Utils.toArray(args))));
      };
  ...
  var Utils = utils; // Append elements of @src array to @dest array
  ...
  utils.range = function (len, start, inc) {
    var arr = [];
    var v = start === void 0 ? 0 : start;
    var i = inc === void 0 ? 1 : inc;
  ...
  utils.forEach = function (arr, func, ctx) {
    if (!utils.isArrayLike(arr)) {
      throw new Error("#forEach() takes an array-like argument. ".concat(arr));
    }
  ...
  Utils.addThousandsSep = function (str) {
    ...
    while (ins > start) {
      fmt = ",".concat(str.substring(ins, end)).concat(fmt);
      end = ins;
      ins -= 3;
    }
  ...
  Utils.findMedian = function (arr) {
    ...
    var median = Utils.findValueByRank(arr, rank);
  ...
  BinArray.prototype = {
    ...
    readFloat64Array: function readFloat64Array(len) {
      var bytes = len * 8,
          i = this._idx,
          buf = this._buffer;
      var arr; // Inconsistent: first is a view, second a copy...
    ...
    writeCString: function writeCString(str, fixedLen) {
      ...
      var bytesWritten = this.writeString(str, maxChars);
  ...
  */
  ...
  utils.wildcardToRegExp = function (name) {
    ...
    return new RegExp("^".concat(rxp, "$"));
  };

  utils.expandoBuffer = function (constructor, rate) {
    var capacity = 0;
    var k = rate >= 1 ? rate : 1.2;
    var buf;
  ...
  var T = {
    ...
    stop: function stop(note) {
      ...
      var msg = "".concat(elapsed, "ms");

      if (note) {
        msg = "".concat(note, " ").concat(msg);
      }
  ...
  internal.formatStringsAsGrid = function (arr) {
    ...
    return arr.reduce(function (memo, name, i) {
      ...
      return "".concat(memo, "  ").concat(name);
    }, '');
  };
  ...
  internal.requireDataFields = function (table, fields) {
    ...
    if (missingFields.length > 0) {
      stop("Table is missing one or more fields:\n", missingFields, "\nExisting fields:", "\n".concat(internal.formatStringsAsGrid(dataFields)));
    }
  };
  ...
  function nearestPoint(x, y) {
    ...
    for (var i = 0, j = 0, n = arguments.length <= 2 ? 0 : arguments.length - 2; j < n; i++, j += 2) {
      dist = distanceSq(x, y, j + 2 < 2 || arguments.length <= j + 2 ? undefined : arguments[j + 2], j + 1 + 2 < 2 || arguments.length <= j + 1 + 2 ? undefined : arguments[j + 1 + 2]);
  ...
  */
  ...
  internal.calcArcBounds = function (xx, yy, start, len) {
    var i = start | 0;
    var n = isNaN(len) ? xx.length - i : len + i;
  ...
  function ArcCollection() {
    ...
    this.forEachArcSegment = function (arcId, cb) {
      var fw = arcId >= 0,
          ...
          n = _nn[absId],
          step = fw ? 1 : -1;
      var v1 = fw ? _ii[absId] : _ii[absId] + n - 1,
          v2 = v1,
          count = 0;
    ...
    function condenseArcs(map) {
      var goodPoints = 0,
          goodArcs = 0;
      var copyElements = utils.copyElements;
      var k, arcLen;
    ...
    this.arcIsLollipop = function (arcId) {
      ...
      var i = this.indexOfVertex(arcId, 1);
      var j = this.indexOfVertex(arcId, -2);
    ...
    function flattenThresholds(arr, n) {
      ...
      arr.forEach(function (arr) {
        for (var j = 0, _n = arr.length; j < _n; i++, j++) {
          zz[i] = arr[j];
        }
      });
  ...
  ArcCollection.prototype.inspect = function () {
    ...
    if (n < 50) {
      ...
      str = "[ArcCollection (".concat(this.size(), ")]");
    }
  ...
  internal.getIntersectionKey = function (o) {
    return "".concat(o.a.join(','), ";").concat(o.b.join(','));
  };
  ...
  */
  ...
  internal.groupPolygonRings = function (paths, reverseWinding) {
    ...
    var ringIndex = require('rbush')(); // eslint-disable-line no-undef
    ...
    holes.forEach(function (hole) {
      ...
      var ring, ringId, ringArea, isContained; // Group this hole with the smallest-area ring that contains it.
  ...
  exportAPI = internal; //this.mapshaper = api;
