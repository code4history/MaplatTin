...
(function () {
    ...
    utils.some = function (arr, test) {
        return arr.reduce((val, item) => val || test(item), false);
    };
    ...
    utils.getArrayBounds = function (arr) {
        let min = Infinity, max = -Infinity, nan = 0, val;
    ...
    Utils.addThousandsSep = function (str) {
        ...
        let end = str.length, ins = (dec == -1 ? end : dec) - 3;
        while (ins > start) {
    ...
    Bounds.prototype.fillOut = function (aspect, focusX, focusY) {
        ...
        const w = this.width(), h = this.height(), currAspect = w / h;
        let pad;
    ...
    Utils.sortOnKey = function (arr, getter, asc) {
        const compare = Utils.getGenericComparator(asc !== false);
    ...
    Utils.quicksortPartition = function (a, lo, hi) {
        ...
        while (i < hi) {
            ...
            if (lo < j)
                Utils.quicksortPartition(a, lo, j);
            lo = i;
            j = hi;
        }
    };
    ...
    Utils.findValueByRank = function (arr, rank) {
        if (!arr.length || rank < 1 || rank > arr.length)
            error("[findValueByRank()] invalid input");
        rank = Utils.clamp(rank | 0, 1, arr.length);
        const k = rank - 1, n = arr.length;
        let l = 0, m = n - 1, i, j, val, tmp;
        while (l < m) {
            val = arr[k];
            i = l;
            j = m;
    ...
    Utils.findMedian = function (arr) {
        const n = arr.length, rank = Math.floor(n / 2) + 1;
    ...
    BinArray.bufferCopy = function (dest, destId, src, srcId, bytes) {
        ...
        const wordSize = Math.min(BinArray.uintSize(bytes), BinArray.uintSize(srcId), BinArray.uintSize(dest.byteLength), BinArray.uintSize(destId), BinArray.uintSize(src.byteLength));
        const srcArr = BinArray.bufferToUintArray(src, wordSize), destArr = BinArray.bufferToUintArray(dest, wordSize);
        let count = bytes / wordSize, i = srcId / wordSize, j = destId / wordSize;
        while (count--) {
            destArr[j++] = srcArr[i++];
        }
        return bytes;
    ...
    BinArray.toArrayBuffer = function (src) {
        const n = src.length, dest = new ArrayBuffer(n), view = new Uint8Array(dest);
        for (let i = 0; i < n; i++) {
            view[i] = src[i];
        }
        return dest;
    ...
    BinArray.prototype = {
        ...
        readFloat64Array(len) {
            const bytes = len * 8, i = this._idx, buf = this._buffer;
            let arr;
            if (i % 8 === 0) {
        ...
        readCString(fixedLen, asciiOnly) {
            let str = "", count = fixedLen >= 0 ? fixedLen : this.bytesLeft();
            while (count > 0) {
        ...
        writeString(str, maxLen) {
            let bytesWritten = 0, charsToWrite = str.length, cval;
            if (maxLen) {
    ...
    function formatValue(val, matches) {
        ...
        const isString = type == "s", isHex = type == "x" || type == "X", isInt = type == "d" || type == "i", isNumber = !isString;
        let sign = "", padDigits = 0, isZero = false, isNeg = false;
    ...
    Utils.formatter = function (fmt) {
        ...
        const literals = [], formatCodes = [];
        let startIdx = 0, prefix = "", matches = codeRxp.exec(fmt), literal;
        while (matches) {
    ...
    internal.formatStringsAsGrid = function (arr) {
        const longest = arr.reduce((len, str) => Math.max(len, str.length), 0), colWidth = longest + 2, perLine = Math.floor(80 / colWidth) || 1;
    ...
    internal.logArgs = function (args) {
        if (internal.LOGGING &&
            !internal.getStateVar("QUIET") &&
            utils.isArrayLike(args)) {
    ...
    internal.probablyDecimalDegreeBounds = function (b) {
        const world = internal.getWorldBounds(-1), bbox = b instanceof Bounds ? b.toArray() : b;
    ...
    internal.requireDataFields = function (table, fields) {
        ...
        const dataFields = table.getFields(), missingFields = utils.difference(fields, dataFields);
    ...
    function signedAngle(ax, ay, bx, by, cx, cy) {
        ...
        const abx = ax - bx, aby = ay - by, cbx = cx - bx, cby = cy - by, dotp = abx * cbx + aby * cby, crossp = abx * cby - aby * cbx, a = Math.atan2(crossp, dotp);
    ...
    function bearing(lng1, lat1, lng2, lat2) {
        ...
        lat1 *= D2R;
        lat2 *= D2R;
        const y = Math.sin(lng2 - lng1) * Math.cos(lat2), x = Math.cos(lat1) * Math.sin(lat2) -
    ...
    function signedAngleSph(alng, alat, blng, blat, clng, clat) {
        ...
        const b1 = bearing(blng, blat, alng, alat), b2 = bearing(blng, blat, clng, clat), a = Math.PI * 2 + b1 - b2;
        return standardAngle(a);
    }
    ...
    function xyzToLngLat(x, y, z, p) {
        const d = distance3D(0, 0, 0, x, y, z);
    ...
    function sphericalDistance(lam1, phi1, lam2, phi2) {
        const dlam = lam2 - lam1, dphi = phi2 - phi1, a = Math.sin(dphi / 2) * Math.sin(dphi / 2) +
            Math.cos(phi1) *
                Math.cos(phi2) *
                Math.sin(dlam / 2) *
                Math.sin(dlam / 2), c = 2 * Math.atan2(Math.sqrt(a), Math.sqrt(1 - a));
        return c;
    }
    function greatCircleDistance(lng1, lat1, lng2, lat2) {
        const D2R = Math.PI / 180, dist = sphericalDistance(lng1 * D2R, lat1 * D2R, lng2 * D2R, lat2 * D2R);
        return dist * R;
    }
    function innerAngle(ax, ay, bx, by, cx, cy) {
        const ab = distance2D(ax, ay, bx, by), bc = distance2D(bx, by, cx, cy);
        let theta, dotp;
        if (ab === 0 || bc === 0) {
            theta = 0;
        }
    ...
    function innerAngle3D(ax, ay, az, bx, by, bz, cx, cy, cz) {
        const ab = distance3D(ax, ay, az, bx, by, bz), bc = distance3D(bx, by, bz, cx, cy, cz);
        let theta, dotp;
        if (ab === 0 || bc === 0) {
            theta = 0;
        }
    ...
    function triangleArea3D(ax, ay, az, bx, by, bz, cx, cy, cz) {
        const area = 0.5 *
            Math.sqrt(detSq(ax, ay, bx, by, cx, cy) +
    ...
    function pointSegDistSq(ax, ay, bx, by, cx, cy) {
        const ab2 = distanceSq(ax, ay, bx, by), ac2 = distanceSq(ax, ay, cx, cy), bc2 = distanceSq(bx, by, cx, cy);
        return apexDistSq(ab2, ac2, bc2);
    }
    function pointSegDistSq3D(ax, ay, az, bx, by, bz, cx, cy, cz) {
        const ab2 = distanceSq3D(ax, ay, az, bx, by, bz), ac2 = distanceSq3D(ax, ay, az, cx, cy, cz), bc2 = distanceSq3D(bx, by, bz, cx, cy, cz);
        return apexDistSq(ab2, ac2, bc2);
    }
    ...
    function FilteredArcIter(xx, yy, zz) {
        let _zlim = 0, _i = 0, _inc = 1, _stop = 0;
        ...
        this.hasNext = function () {
            const zarr = zz, i = _i;
            let j = i;
            const zlim = _zlim, stop = _stop, inc = _inc;
            if (i == stop)
                return false;
            do {
                j += inc;
    ...
    function ArcCollection(...args) {
        let _xx, _yy, _ii, _nn, _zz, _zlimit = 0, _bb, _allBounds, _arcIter, _filteredArcIter;
        if (args.length == 1) {
            initLegacyArcs(args[0]);
        }
        else if (args.length == 3) {
        ...
        this.getCopy = function () {
            const copy = new ArcCollection(new Int32Array(_nn), new Float64Array(_xx), new Float64Array(_yy));
            if (_zz) {
        ...
        function getFilteredVertexData() {
            ...
            const xx2 = new Float64Array(len2), yy2 = new Float64Array(len2), zz2 = new Float64Array(len2), nn2 = new Int32Array(arcCount);
            let i = 0, i2 = 0, n, n2;
        ...
        this.forEachArcSegment = function (arcId, cb) {
            const fw = arcId >= 0, absId = fw ? arcId : ~arcId, zlim = this.getRetainedInterval(), n = _nn[absId], step = fw ? 1 : -1;
            let v1 = fw ? _ii[absId] : _ii[absId] + n - 1, v2 = v1, count = 0;
            for (let j = 1; j < n; j++) {
                v2 += step;
        ...
        this.deleteArcs = function (test) {
            const n = this.size(), map = new Int32Array(n);
            let goodArcs = 0;
            for (let i = 0; i < n; i++) {
                if (test(i)) {
                    map[i] = goodArcs++;
        ...
        this.dedupCoords = function () {
            ...
            const arcCount = this.size(), zz = _zz;
            let arcLen, arcLen2;
        ...
        this.indexOfVertex = function (arcId, nth) {
            const absId = arcId < 0 ? ~arcId : arcId, len = _nn[absId];
            if (nth < 0)
                nth = len + nth;
            if (absId != arcId)
                nth = len - nth - 1;
        ...
        this.arcIsClosed = function (arcId) {
            const i = this.indexOfVertex(arcId, 0), j = this.indexOfVertex(arcId, -1);
        ...
        this.getArcIter = function (arcId) {
            const fw = arcId >= 0, i = fw ? arcId : ~arcId, iter = _zz && _zlimit ? _filteredArcIter : _arcIter;
            if (i >= _nn.length) {
        ...
        this.getRemovableThresholds = function (nth) {
            ...
            const skip = nth | 1, arr = new Float64Array(Math.ceil(_zz.length / skip));
            let z, i, j, n;
        ...
        this.getArcThresholds = function (arcId) {
            ...
            const start = _ii[arcId], end = start + _nn[arcId];
    ...
    geom.findClosestPointOnSeg = function (px, py, ax, ay, bx, by) {
        const dx = bx - ax, dy = by - ay, dotp = (px - ax) * dx + (py - ay) * dy, abSq = dx * dx + dy * dy, k = abSq === 0 ? -1 : dotp / abSq, eps = 0.1;
        let p;
        if (k <= eps) {
            p = [ax, ay];
        }
    ...
    function segmentHit(ax, ay, bx, by, cx, cy, dx, dy) {
        return (orient2D(ax, ay, bx, by, cx, cy) * orient2D(ax, ay, bx, by, dx, dy) <=
            0 &&
    ...
    internal.quicksortSegmentIds = function (a, ids, lo, hi) {
        ...
        while (i < hi) {
            ...
            if (j - lo < 40)
                internal.insertionSortSegmentIds(a, ids, lo, j);
            else
                internal.quicksortSegmentIds(a, ids, lo, j);
            if (hi - i < 40) {
    ...
    internal.findSegmentIntersections = (function () {
        ...
        return function (arcs) {
            const bounds = arcs.getBounds(), ymin = bounds.ymin, yrange = bounds.ymax - ymin, stripeCount = internal.calcSegmentIntersectionStripeCount(arcs), stripeSizes = new Uint32Array(stripeCount), stripeId = stripeCount > 1 ? multiStripeId : singleStripeId;
            let i, j;
            ...
            const raw = arcs.getVertexData(), intersections = [];
            let arr;
    ...
    internal.calcSegmentIntersectionStripeCount = function (arcs) {
        const yrange = arcs.getBounds().height(), segLen = internal.getAvgSegment2(arcs)[1];
        let count = 1;
    ...
    internal.intersectSegments = function (ids, xx, yy) {
        ...
        internal.sortSegmentIds(xx, ids);
        i = 0;
        while (i < lim) {
            s1p1 = ids[i];
            s1p2 = ids[i + 1];
    ...
    internal.getAvgSegment = function (arcs) {
        ...
        const count = arcs.forEachSegment((i, j, xx, yy) => {
            const dx = xx[i] - xx[j], dy = yy[i] - yy[j];
    ...
    internal.findShapesByArcId = function (shapes, arcIds, numArcs) {
        const index = numArcs ? new Uint8Array(numArcs) : [], found = [];
        arcIds.forEach(id => {
            index[absArcId(id)] = 1;
        });
    ...
    internal.findNextRemovableVertices = function (zz, zlim, start, end) {
        let i = internal.findNextRemovableVertex(zz, zlim, start, end), arr, k;
        if (i > -1) {
            k = zz[i];
            arr = [i];
            while (++i < end) {
    ...
    internal.groupPolygonRings = function (paths, reverseWinding) {
        const holes = [], groups = [], sign = reverseWinding ? -1 : 1;
        (paths || []).forEach(path => {
            if (path.area * sign > 0) {
                groups.push([path]);
            }
        ...
        holes.forEach(hole => {
            let containerId = -1, containerArea = 0;
            const holeArea = hole.area * -sign, candidates = ringIndex.search({
    ...
    internal.quantizeArcs = function (arcs, quanta) {
        const bb1 = arcs.getBounds(), bb2 = new Bounds(0, 0, quanta - 1, quanta - 1), fw = bb1.getTransform(bb2), inv = fw.invert();
...
//# sourceMappingURL=mapshaper-maplat.js.map